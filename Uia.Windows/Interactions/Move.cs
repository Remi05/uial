...
namespace Uial.Windows.Interactions
{
    public class Move : AbstractPatternInteraction<IUIAutomationTransformPattern>, IInteraction
    {
        ...
        public static Move FromRuntimeValues(IWindowsVisualContext context, IEnumerable<object> paramValues)
        {
            if (paramValues.Count() != 2)
            {
                throw new InvalidParameterCountException(2, paramValues.Count());
            }
            double x = double.Parse(paramValues.ElementAt(0) as string);
            double y = double.Parse(paramValues.ElementAt(1) as string);
